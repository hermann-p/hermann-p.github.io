...
(defn mk-view [width height & base]
  (let [renderer (pixi.autoDetectRenderer width height (clj->js {:transparent true}))
        stage (pixi.Container.)]
    (set! (.-autoResize renderer) true)
    (set! (.-transparent renderer) true)
    (dommy/append! (dommy/sel1 (if base base :body))
                   (-> (.-view renderer)
...
(defmulti rotate! dispatch)
(defmethod rotate! :wrapped [el angle]
  (update-in el [:sprite] #(rotate! % angle)))
(defmethod rotate! :raw [el angle]
  (let [cur-angle (.-rotation el)]
    (set! (.-rotation el) (+ cur-angle angle))))
      ...
      (when (seq args)
        (let [op (first args)
          ...
          (case op
            ...
            :rotate (rotate! sprite (first params))
...
(defn mk-sprite [tex & args]
  (apply update-sprite! (pixi.Sprite. tex) args))

(defn update-pos! [{:keys [x y sprite] :as el}]
  (set-pos el :x x :y y))
